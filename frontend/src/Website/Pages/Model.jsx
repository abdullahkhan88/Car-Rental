    ...
    <>
      {show && (
        <div className="fixed inset-0 bg-opacity-30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="relative w-[90%] max-w-4xl bg-white bg-opacity-80 rounded-xl shadow-2xl flex flex-col md:flex-row overflow-hidden transition-all duration-500">

            {/* Background shade transition layer */}
            <div
              className={`absolute w-full md:w-1/2 h-1/2 md:h-full bg-gradient-to-br from-blue-400 to-blue-600 transition-all duration-700 ease-in-out z-0
              ${activePanel === "left" ? "top-1/2 md:top-0 md:left-1/2" : "top-0 md:top-0 md:left-0"}
              `}
            ></div>

            {/* Left Panel */}
            <div
              className={`relative z-10 w-full md:w-1/2 p-6 transition-transform duration-700
              ${activePanel === "left"
            ...
            >
              ...
              <div className="flex gap-4 justify-center mt-3">
                {["green-500", "red-500", "blue-500", "yellow-400"].map((color, i) => (
                  <input
                    key={i}
                    type="text"
                    maxLength={1}
                    className={`w-12 h-12 p-2 text-xl text-center text-${color} border-3 border-blue-200 rounded mb-2 focus:outline-none shadow-sm`}
                  />
                ))}
              </div>
              ...
              <div className="mt-1 text-center">
                <p className="text-green-400">Verifications Successfully</p>
              </div>
